...
void ai_util_RecastDetour::cleanup()
{
...
bool ai_util_RecastDetour::handleBuild(scene::IMeshSceneNode* node)
{
    cleanup();
    ...
    if (!this->getMeshBufferData(node, vs, nverts, ts, ntris))
    {
...
bool ai_util_RecastDetour::getMeshBufferData
(
...
)
{
    if (!node)
    {
        printf("getMeshBufferData: node is null\n");
        return false;
    }
    ...
    if (!mesh)
    {
        printf("getMeshBufferData: mesh is null\n");
        return false;
    }
    ...
    if ((nverts <= 0) || (ntris <= 0))
    {
        printf("getMeshBufferData: No vertices or triangles found in any buffer.\n");
        return false;
    }
    ...
    printf("getMeshBufferData: Total nverts (components)=%i\n", nverts);
    printf("getMeshBufferData: Total ntris=%i\n", ntris);

    return true;
}


bool ai_util_RecastDetour::setupIrrSMeshFromRecastDetailMesh(scene::SMesh* smesh)
{
...
void ai_util_RecastDetour::showHeightFieldInfo(const rcHeightfield& hf)
{
    int w = hf.width;
    int h = hf.height;
...
bool ai_util_RecastDetour::getMeshDataFromPolyMeshDetail(rcPolyMeshDetail* dmesh, std::vector<float>& vertsOut, int& nvertsOut, std::vector<int>& trisOut, int& ntrisOut)
{
...
bool ai_util_RecastDetour::setMeshBufferData(scene::SMeshBuffer& buffer, const std::vector<float>& verts, int& nverts, const std::vector<int>& tris, int& ntris)
{
    //TODO:
    // err handling:
...
//Detour stuff
vector<irr::core::vector3df>  ai_util_RecastDetour::returnPath(core::vector3df pStart, core::vector3df pEnd)
{
